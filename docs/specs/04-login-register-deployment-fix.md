...
- 첫 새 Services 배포의 `/api/auth/me`는 정상적인 401 JSON으로 복구됐으나, 운영 회원가입은 503이었다. 내부 로그는 `OperationalError`, `sqlstate=None`, `endpoint=supabase_direct`를 기록했다. 로그와 Supabase 연결 안내에 비춰 Vercel의 IPv4 네트워크에서 직접 연결(IPv6)을 사용하는 것이 가장 유력한 원인이었다.
- 운영 `DATABASE_URL`을 수정하고 다시 배포한 뒤 회원가입 API는 201로 성공했다. 가입 직후 이동하는 홈(`/`)은 Vercel 404였다. 기존 프론트 catch-all `/:path*`가 루트 경로를 포함하지 않아 `/(.*)`로 바꿨다.
...
1. Vercel 프로젝트 Root Directory를 저장소 루트로, Framework Preset을 `Services`로 설정한다. 루트 `vercel.json`이 FastAPI와 Next.js를 함께 빌드하고 `/api/*`를 backend 서비스로, 루트(`/`)를 포함한 나머지 경로를 frontend 서비스로 전달하는지 배포 출력과 HTTP 응답에서 확인한다.
...
- 최종 Services Production 배포 `dpl_HwJnVf2Xm5QSz9fMkRyZXhYa8GYi`를 공개 alias `https://gamja-market-pi.vercel.app`에 적용했다. 공개 홈(`/`)은 200, 쿠키 없는 `/api/auth/me`는 FastAPI의 401 JSON을 반환했다.
- Orca 내장 브라우저에서 새 테스트 계정으로 회원가입했다. `/api/auth/signup`은 201, 자동 로그인 뒤 이동한 홈은 200이었다. 화면에 계정 이메일과 로그아웃 버튼이 나타났고 새로고침 뒤 `/api/auth/me` 200으로 세션이 복원됐다.
- `/api/auth/logout`은 204였고 화면에 로그인·회원가입 링크가 다시 나타났다. 같은 계정으로 `/api/auth/login` 200, 홈 200, 새로고침 뒤 `/api/auth/me` 200과 계정 상태 복원을 확인했다. 테스트 자격 증명은 문서에 기록하지 않는다.
